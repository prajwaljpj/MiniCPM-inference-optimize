...
class MiniCPMO(MiniCPMOPreTrainedModel):
    ...
    def init_tts(
        ...
        if tts_text_tokenizer_path is None:
            tts_text_tokenizer_path = os.path.join(
                self.config._name_or_path, "assets/chattts_tokenizer"
            )
    ...
    def initialize_vocos(self, ckpt_path):
        ...
        vocos = (
            Vocos(feature_extractor, backbone, head).to("cuda").eval().to(torch.float32)
        )
    ...
    def get_audio_embedding(self, data, chunk_length=-1, dummy=True):
        ...
        if len(wavforms) > 0:
            ...
            audio_states = self.apm(
                dummy_wavs, output_hidden_states=True
            ).hidden_states[self.audio_encoder_layer]
    ...
    def get_omni_embedding(
        self, data, input_embeddings, chunk_length=-1, stream_input=False
    ):
        """
        Args:
            data:
    ...
    def get_sys_prompt(self, ref_audio=None, mode="default", language="zh"):
        ...
        if mode == "omni":
            if language == "zh":
                ...
                vc_prompt_prefix = (
                    sys_prompt + "Clone the voice in the provided audio prompt."
                )
                vc_prompt_suffix = (
            ...
            if ref_audio is not None:
                sys_msgs = {
                    "role": "user",
                    "content": [vc_prompt_prefix, ref_audio, vc_prompt_suffix],
                }
            ...
            if language == "zh":
                ...
                vc_prompt_suffix = (
                    "Try to role-play the character based on the audio prompt above."
                )
        ...
        with torch.inference_mode():
            ...
            if stream:
                result = self._decode_stream(
                    model_inputs["inputs_embeds"], tokenizer, **kwargs
                )
                # if stream return TextIteratorStreamer and output is empty
                outputs = {}
            else:
                outputs = self._decode(
                    model_inputs["inputs_embeds"], tokenizer, attention_mask, **kwargs
                )
        ...
        if stream:
            ...
            if batched:
                ...
                if use_tts_template and generate_audio:
                    ...
                    wav_numpy, sr = self.decode_mel_to_audio(
                        mel_spec, output_audio_path
                    )
    ...
    def streaming_prefill(
        ...
        **kwargs,
        ...
        if (
            not self.is_first and self.new_user_msg and msg["role"] == "user"
        ):  # new user add im_start
    ...
    def streaming_generate(
        ...
        **kwargs,
        ...
        input_ids = tokenizer(
            generate_prompt, return_tensors="pt", add_special_tokens=False
        )["input_ids"].cuda()
        ...
        attention_mask = torch.ones(
            (1, cache_length + input_ids.shape[1]), dtype=torch.bool, device=self.device
        )

        generation_config["max_new_tokens"] = max_new_tokens
        streamer = self.llm_generate_chunk(
            input_ids, attention_mask, tokenizer, terminators, generation_config
        )
    ...
    def llm_generate_chunk(
        ...
        while True:
            ...
            if new_len >= max_new_tokens:
                logger.debug(
                    f"LLM generation {new_len} exceeds max_new_tokens({max_new_tokens}), break."
                )
                break

    def prepare_tts_text(self, text):
        tts_tokens = self.tts_processor.text_tokenizer.encode(
        ...
        if tts_tokens_len < self.tts.streaming_text_reserved_len:
            ...
            tts_tokens_len = len(tts_tokens)
            text = self.tts_processor.text_tokenizer.decode(
                tts_tokens, add_special_tokens=False
            )
            pad_str = ""
    ...
    def _build_streaming_mask(self, tts_tokens_len):
        tts_sequence_full_length = (
            1
            + self.tts.num_spk_embs * self.tts.use_speaker_embedding
            + self.tts.streaming_text_reserved_len
            + 1
        )
        streaming_attention_mask = torch.zeros(
            tts_sequence_full_length, dtype=torch.int8
        )
    ...
    def _generate_mel_spec(
        ...
        tts_inputs = self.tts_processor.text_tokenizer.encode(
            tts_text, add_special_tokens=False
        )
        tts_input_ids = (
            torch.Tensor(tts_inputs).unsqueeze(0).to("cuda", dtype=torch.long)
        )
        streaming_tts_text_mask = self._build_streaming_mask(tts_token_lens).to(
        ...
        emb = torch.zeros(
            1, condition_length, self.tts.num_vq, dtype=dtype, device=self.tts.device
        )
        past_key_values = [
            (
                torch.zeros(
        ...
        for chunk_idx in range(
            math.ceil(emb.shape[1] / self.tts.streaming_text_chunk_size)
        ):
            if chunk_idx == 0:
            ...
            if end - begin > 0:
                ...
                position_ids = torch.arange(
                    begin, end, dtype=torch.long, device=self.tts.device
                ).unsqueeze(0)
            ...
            outputs = self.tts.generate(
                ...
                temperature=torch.tensor(
                    [0.1, 0.3, 0.1, 0.3], dtype=torch.float, device=self.tts.device
                ),
            ...
            while True:
                outputs = self.tts.generate(
                    ...
                    eos_token=torch.tensor(
                        [625], dtype=torch.long, device=self.tts.device
                    ),
                    logits_warpers=logits_warpers,
                    logits_processors=logits_processors,
                )
    ...
    def _generate_mel_spec_audio_streaming(
        ...
        if spk_embeds is None:
            ...
            tts_text, tts_token_lens = self.prepare_tts_text(gen_text)
            last_hidden_states = r["hidden_states"][0][-1][
                0
            ]  # output: (input_seq_len, dim)
            spk_embeds = last_hidden_states[spk_bound[0] : spk_bound[1]]
        ...
        if prev_seg_text_ids is not None and prev_seg_audio_ids is not None:
            ...
            position_ids = torch.arange(
                0,
                tts_token_lens + tts_start_token_len,
                dtype=torch.long,
                device=self.tts.device,
            ).unsqueeze(0)
            ...
            text = self.tts_processor.text_tokenizer.decode(
                prev_seg_text_ids[0].tolist(), add_special_tokens=False
            )
        ...
        for t, r in enumerate(streamer):
            ...
            if tts_token_lens >= (chunk_idx + 1) * self.tts.streaming_text_chunk_size:
                ...
                mel_spec = self.tts.decode_to_mel_specs(
                    outputs.new_ids[:, max(new_ids_len - 4, 0) :, :]
                )
                new_ids_len = outputs.new_ids.shape[1]  # [1, seq_len, 4]

                wav_np, sr = self.decode_mel_to_audio(
                    mel_spec
                )  # [1,100,50] -> [50*256]
                ...
                if enable_regenerate and voice_checker.is_bad(
                    check_wav_np, check_mel, chunk_size=2560
                ):
                    voice_checker.reset()
                    # regenerate
                    ...
                    if prev_wav is not None:
                        ...
                        yield OmniOutput(
                            text=cur_text, audio_wav=wav_y, sampling_rate=sr
                        )
                    ...
                    if prev_wav is not None:
                        ...
                        prev_text_len = len(gen_text_raw)
                        yield OmniOutput(
                            text=cur_text, audio_wav=wav_np, sampling_rate=sr
                        )
        ...
        if not eos_lab and tts_text:
            ...
            if chunk_idx == 0:
                ...
                begin = (
                    chunk_idx * self.tts.streaming_text_chunk_size + tts_start_token_len
                )
        ...
        if new_segment_gen and not stop:
            ...
            prev_seg_text_ids = tts_input_ids[
                :, end - 1 - tid_len : end - 1
            ]  # exclude last Etts
